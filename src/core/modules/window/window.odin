...
title_display_FPS :: proc(FRAME_DATA: vault.FrameData, window_title: cstring, window: ^glfw.WindowHandle) -> (fData: vault.FrameData) {
    FDATA        := FRAME_DATA
    currentTime  := glfw.GetTime()
    FDATA.frame_count += 1

    if (currentTime - FDATA.previous_time >= 1.0) {
        frameString  := fmt.tprintf("%s - %d FPS", window_title, FDATA.frame_count)
