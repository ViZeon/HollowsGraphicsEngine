...
model_load_realtime :: proc() {
    ...
    cached_model, cache_ok := load_model_cache(data.MODEL_PATH, data.SCALE_FACTOR)
    
    if cache_ok {
        fmt.println("✓ Loaded model from cache")
        data.MODEL_DATA = cached_model
        data.MODEL_INITIALIZED = true
        return
    }
    
    // Cache miss - load and process normally
    fmt.println("Cache miss - loading model from file...")
    ...
    if save_model_cache(&data.MODEL_DATA, data.MODEL_PATH, data.SCALE_FACTOR) {
        fmt.println("✓ Model cached for future loads")
    }
...
grid_spatial_populate :: proc(
    ...
    // Try loading from cache first
    if load_grid_cache(cells, data.MODEL_PATH) {
        fmt.println("✓ Loaded spatial grid from cache")
        return  // ← Make sure this returns BEFORE the rest of the code
    }
    
    fmt.println("Cache miss - building spatial grid...")
    ...
    // Directional sweep
    sweep_direction(cells, size_x, size_y, size_z)
    
    // Save to cache
    if save_grid_cache(cells, data.MODEL_PATH) {
        fmt.println("✓ Spatial grid cached for future loads")
    }
}
