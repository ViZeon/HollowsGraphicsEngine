...
debug_frame_end :: proc() {
    ...
    if current_time - debug_stats.last_print_time > 2.0 {
        avg_vertices_per_pixel := f32(debug_stats.total_vertices_checked) / f32(debug_stats.total_pixels_processed)
        frame_avg := f32(debug_stats.frame_vertices_checked) / f32(debug_stats.frame_pixels_processed)
...
debug_draw_overlay :: proc() {
    if rl.IsKeyDown(.F1) {
        ...
        rl.DrawText(fmt.ctprintf("Cam: [%.0f, %.0f, %.0f]", 
                    data.CAM_POS.x, data.CAM_POS.y, data.CAM_POS.z), 20, y, 20, rl.WHITE)
        y += 25
        ...
        rl.DrawText(fmt.ctprintf("Range: %d-%d", 
                    debug_stats.min_range_size, debug_stats.max_range_size), 20, y, 20, rl.WHITE)
