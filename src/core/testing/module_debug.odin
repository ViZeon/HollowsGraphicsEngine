...
debug_init :: proc() {
    ...
    stats.last_cam_pos    = (^math.vec3)(data.edit(vault.cam_pos))^
    save_screen_pos()
}
...
debug_build_snapshot :: proc(stats: ^vault.Debug_Stats) -> string {
    ...
    fmt.sbprintf(&sb, "=== HOLLOWS ENGINE ===\n")
    fmt.sbprintf(&sb, "FPS: %d | Pixel: %.1fms | Input: %.3fms | Texture: %.3fms\n",
        fps,
        stats.pixel_time   * 1000,
        stats.input_time   * 1000,
        stats.texture_time * 1000,
    )
    fmt.sbprintf(&sb, "Camera: [%.2f, %.2f, %.2f]\n\n", cam_pos.x, cam_pos.y, cam_pos.z)

    fmt.sbprintf(&sb, "--- DATA (valid / total) ---\n")
    grand_valid := 0
    grand_total := 0
    for type_id in vault.Type_ID {
        ...
        grand_valid += valid
        grand_total += total
        fmt.sbprintf(&sb, "  %-18v %d / %d\n", type_id, valid, total)
    }
    fmt.sbprintf(&sb, "  %-18v %d / %d\n\n", "TOTAL", grand_valid, grand_total)

    fmt.sbprintf(&sb, "--- FETCHES (edit / copy / set) ---\n")
    for type_id in vault.Type_ID {
